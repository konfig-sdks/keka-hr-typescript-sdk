...
import { readableStreamToString, KekaHrError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://login.kekad.com/connect/authorize"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new KekaHrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new KekaHrError(e, e.response?.data)
                }
                if (innerError instanceof KekaHrError) {
                    // Got "KekaHrError" from the above try block
                    throw innerError;
                }
